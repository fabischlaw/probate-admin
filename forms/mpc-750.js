...
const COURT_URL = 'https://courtforms.jud.state.ma.us/publicforms/PFC0026';

// MPC-750 is mostly court-completed. We pre-fill the header, PR name, and
// suggest testate/intestate so the form is ready for the magistrate.
async function fillMPC750(matter, contacts, assets, toggleAnswers = {}) {
  ...
  const warnings = [];
  if (!dec) warnings.push('MPC-750: No decedent contact found');
  ...
  await page.evaluate((d) => {
    ...
    // Testate / intestate suggestion (court confirms)
    set('CheckBox1_21', !!d.toggles.hasWill);   // testate
    set('CheckBox1_18', !d.toggles.hasWill);     // intestate
    if (d.toggles.hasWill && d.willDate) {
      set('TextField4_23', d.willDate);
    }

    // Informal probate / appointment suggestions
    set('CheckBox1_41', true);   // informal probate granted
    set('CheckBox1_43', true);   // appointment granted

    // PR reference
    set('TextField4_44', d.rep?.last_name);
    set('TextField4_50', d.rep?.first_name);

    // Bond amount
    if (d.bondAmount) {
      set('PenalSumAmntNfd_102', d.bondAmount);
    }
  }, {
    ...
    bondAmount: toggleAnswers.bondAmount || '',
